...
static void DrawText(Bitu x,Bitu y,const char * text,Bit8u color) {
	SDL_Surface * s = SDL_GetWindowSurface(mapper.surface);

	Bit8u * draw=((Bit8u *)s->pixels)+(y*s->pitch)+x;
	while (*text) {
...
public:
	...
	virtual void Draw(void) {
		...
		Bit8u * point=((Bit8u *)s->pixels)+(y*s->pitch)+x;
...
public:	
	...
	void Draw(void) {
		...
		if (checked) {
			...
			Bit8u * point=((Bit8u *)s->pixels)+((y+2)*s->pitch)+x+dx-dy+2;
...
static void DrawButtons(void) {
	...
	SDL_UnlockSurface(s);
	SDL_UpdateWindowSurface(mapper.surface);
}
...
void MAPPER_RunInternal() {
	...
	GFX_EndUpdate( 0 );
	mapper.surface=SDL_SetVideoMode_Wrap(DEFAULT_WIDTH,DEFAULT_HEIGHT,8,0);
